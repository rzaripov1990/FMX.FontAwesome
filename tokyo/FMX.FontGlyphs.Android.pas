...
type
  ...
  protected
    ...
    function DoGetGlyph(const Char: UCS4Char;
      const Settings: TFontGlyphSettings; const UseColorfulPalette: Boolean)
      : TFontGlyph; override;
...
uses
  ...
  Androidapi.JNIBridge, Androidapi.Helpers, System.IOUtils;
...
function TAndroidFontGlyphManager.IsColorfulCharacter
  (const Char: UCS4Char): Boolean;
begin
  Result := inherited or ((Char >= $2600) and (Char <= $26FF) and
    (Char <> $267B) and (Char <> $2660) and (Char <> $2665) and (Char <> $2663)
    and (Char <> $2666) and (Char <> $26AA) and (Char <> $26AB)) or
    ((Char >= $2700) and (Char <= $27BF) and (Char <> $27A1) and (Char <> $2733)
    and (Char <> $2747) and (Char <> $2734) and (Char <> $2757) and
    (Char <> $2716) and (Char <> $2714)) or
    ((Char >= $3000) and (Char <= $303F) and (Char <> $3030)) or (Char = $1F201)
    or (Char = $1F202) or ((Char >= $1F300) and (Char <= $1F64F)) or
    ((Char >= $1F680) and (Char <= $1F6FF));
end;
{$ENDIF}
